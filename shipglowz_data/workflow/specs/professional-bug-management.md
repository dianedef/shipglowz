...
supersedes: []
evidence:
  - "User feedback 2026-04-27: shipglowz_data/workflow/TEST_LOG.md alone will grow without bound if it stores full bug investigation detail."
  - "User feedback 2026-04-27: bug traceability must include how the bug happens and what was attempted to fix it."
  - "Repo investigation: sg-test currently writes shipglowz_data/workflow/TEST_LOG.md and shipglowz_data/workflow/BUGS.md, but has no per-bug dossier or evidence directory."
...
- User/operator result: l'utilisatrice voit un identifiant stable `BUG-YYYY-MM-DD-NNN`, un `shipglowz_data/workflow/BUGS.md` compact, un dossier `shipglowz_data/workflow/bugs/BUG-ID.md`, les preuves sous `test-evidence/BUG-ID/` quand necessaire, un statut canonique et la prochaine commande claire.
...
- Success proof: ouvrir `shipglowz_data/workflow/BUGS.md` donne l'etat global en moins d'une minute; ouvrir `shipglowz_data/workflow/bugs/BUG-ID.md` donne assez de contexte pour qu'un agent frais reproduise, corrige, reteste ou refuse une fermeture dangereuse sans lire l'historique de conversation.
- Silent success: not allowed; toute creation, mise a jour, retest, blocage de ship ou fermeture de bug doit etre visible dans `shipglowz_data/workflow/BUGS.md`, dans le dossier bug ou dans le rapport final du skill.

## Error Behavior

- Expected failures: bug deja existant, reproduction incomplete, preuve trop volumineuse, preuve sensible non redigee, bug non reproductible, plusieurs bugs similaires, fichier non modifiable, evidence fournie sans chemin, collision d'ID, conflit entre `shipglowz_data/workflow/BUGS.md` et `shipglowz_data/workflow/bugs/`, correction tentee sans retest, ship demande avec bug high/critical ouvert.
...
- System effect: aucun gros log inline; aucune duplication non controlee dans `shipglowz_data/workflow/TEST_LOG.md`; aucun bug marque `closed` sans retest ou exception `closed-without-retest`; aucune preuve brute connue comme sensible ne doit etre persistee; aucun ancien contexte ne doit etre supprime.
...
`sg-test` a deja la bonne intention: transformer un resultat de test en memoire projet durable. Mais le modele actuel melange trop de responsabilites. `shipglowz_data/workflow/TEST_LOG.md` risque de devenir un journal gigantesque si chaque echec contient reproduction, captures, logs, hypotheses, tentatives de fix et retests. `shipglowz_data/workflow/BUGS.md` risque aussi de devenir illisible si chaque bug y accumule tout son historique.
...
Introduire une architecture de bug tracking ShipGlowz en trois couches: `shipglowz_data/workflow/TEST_LOG.md` reste un journal compact des scenarios testes, `shipglowz_data/workflow/BUGS.md` devient un index compact des bugs, et `shipglowz_data/workflow/bugs/BUG-ID.md` devient le dossier complet du bug. Les preuves volumineuses vivent dans `test-evidence/BUG-ID/` quand elles sont redigees et utiles, ou restent en reference externe si elles ne doivent pas etre copiees. Les skills `sg-test`, `sg-fix`, `sg-verify`, `sg-ship`, `sg-docs` et les docs doivent appliquer la meme separation de responsabilites, les memes statuts et les memes regles de preuve.

## Scope In

- Definir la structure projet standard pour bugs: `shipglowz_data/workflow/BUGS.md`, `shipglowz_data/workflow/bugs/`, `test-evidence/`.
- Transformer `shipglowz_data/workflow/BUGS.md` en index compact plutot qu'en dossier complet.
- Creer un format de dossier bug individuel `shipglowz_data/workflow/bugs/BUG-ID.md`.
...
- Definir la generation d'ID et la resolution de collisions entre `shipglowz_data/workflow/BUGS.md`, `shipglowz_data/workflow/bugs/` et plusieurs agents.
...
- `shipglowz_data/workflow/TEST_LOG.md` et `shipglowz_data/workflow/BUGS.md` restent des trackers operationnels courts, sans frontmatter obligatoire.
- `shipglowz_data/workflow/bugs/BUG-ID.md` est un artefact durable ShipGlowz et doit utiliser le frontmatter standard avec `artifact: bug_record`.
- `tools/shipflow_metadata_lint.py` doit accepter `artifact: bug_record` et peut linter `shipglowz_data/workflow/bugs/*.md` quand le chemin est fourni ou quand un dossier `shipglowz_data/workflow/bugs/` existe.
...
- Runtime: markdown, YAML frontmatter pour `shipglowz_data/workflow/bugs/*.md`, conventions de nommage `BUG-YYYY-MM-DD-NNN`, relecture de fichiers avant ecriture.
...
- Metadata gaps: `shipglowz_data/workflow/BUGS.md` and `shipglowz_data/workflow/TEST_LOG.md` intentionally remain tracker files and should not be migrated to artifact frontmatter.
...
- `shipglowz_data/workflow/TEST_LOG.md` answers: what was tested, when, with what result, and which bug or next step followed.
- `shipglowz_data/workflow/BUGS.md` answers: what bugs are open, blocked, duplicate, closed, severity, owner/status if known, last tested date, and link to the detailed dossier.
- `shipglowz_data/workflow/bugs/BUG-ID.md` answers: how the bug reproduces, what evidence exists, what was tried, what changed, and how retests behaved.
...
- Existing `shipglowz_data/workflow/BUGS.md` content remains valid legacy content; new or retested bugs should use the new compact index plus dossier model.
...
- Before assigning an ID, the skill must re-read `shipglowz_data/workflow/BUGS.md` and list `shipglowz_data/workflow/bugs/BUG-YYYY-MM-DD-*.md`, then pick the next numeric suffix greater than every suffix found in either place.
...
- Regression impact: existing projects with simple `shipglowz_data/workflow/BUGS.md` should continue working; the new structure is additive and applies when new bugs are opened or retested.
- Public documentation impact: the website skill pages and README must not describe old `shipglowz_data/workflow/BUGS.md` behavior after the new dossier model lands.
...
- `skills/sg-test/SKILL.md` must explain compact `shipglowz_data/workflow/TEST_LOG.md`, compact `shipglowz_data/workflow/BUGS.md`, per-bug dossiers, ID generation, status lifecycle, evidence storage, redaction, and retest behavior.
...
- ID collision: re-read both `shipglowz_data/workflow/BUGS.md` and `shipglowz_data/workflow/bugs/` immediately before writing; if the chosen ID now exists, increment and retry once. If still conflicting, stop and report the collision.
- `shipglowz_data/workflow/BUGS.md` references a missing dossier: keep index entry, set or report `needs-info`, and do not erase the index.
- Dossier exists without `shipglowz_data/workflow/BUGS.md` entry: add a compact index pointer if the dossier frontmatter is valid; otherwise report metadata gap.
...
- [ ] Task 3: Define compact test and bug tracking doctrine in `sg-test`
  ...
  - Action: Replace the current full `shipglowz_data/workflow/BUGS.md` append format with compact `shipglowz_data/workflow/BUGS.md` index rules, `shipglowz_data/workflow/bugs/BUG-ID.md` dossier rules, `test-evidence/BUG-ID/` rules, canonical statuses, ID generation, duplicate detection and evidence redaction.
  - User story link: Prevents `shipglowz_data/workflow/TEST_LOG.md` and `shipglowz_data/workflow/BUGS.md` from becoming unbounded investigation dumps.
...
- [ ] Task 4: Update `sg-test` retest behavior
  - File: `skills/sg-test/SKILL.md`
  - Action: Specify that `--retest BUG-ID` reads `shipglowz_data/workflow/bugs/BUG-ID.md`, appends retest history there, writes only a short `shipglowz_data/workflow/TEST_LOG.md` pointer, and moves status only through allowed transitions.
...
- [ ] Task 5: Update `sg-fix` bug dossier intake
  ...
  - Action: When invoked with `BUG-ID` or a matching bug title, read `shipglowz_data/workflow/BUGS.md` and `shipglowz_data/workflow/bugs/BUG-ID.md`, reconstruct the bug story from the dossier, append diagnosis notes and fix attempts, and refuse closure without retest evidence.
...
- [ ] Task 6: Update verification gate behavior
  ...
  - Action: Require `sg-verify` to inspect linked `shipglowz_data/workflow/BUGS.md` and `shipglowz_data/workflow/bugs/*.md` for the verified scope, report open high/critical bugs, and block optimistic closure when bug state contradicts the user story.
  - User story link: Prevents verification from claiming success while known core bugs remain unresolved.
  - Depends on: Task 5
  - Validate with: `rg -n "shipglowz_data/workflow/BUGS.md|shipglowz_data/workflow/bugs/|open bug|fixed-pending-verify|high|critical|blocks ship" skills/sg-verify/SKILL.md`
...
- [ ] Task 7: Update shipping gate behavior
  ...
  - Depends on: Task 6
  - Validate with: `rg -n "shipglowz_data/workflow/BUGS.md|shipglowz_data/workflow/bugs/|high|critical|blocked|partial-risk|bug" skills/sg-ship/SKILL.md`
...
- [ ] Task 8: Update documentation coherence skill
  ...
  - Action: Teach docs audit/update mode to recognize the professional bug model, verify docs mention `shipglowz_data/workflow/TEST_LOG.md`, compact `shipglowz_data/workflow/BUGS.md`, per-bug dossiers and evidence redaction, and preserve the rule that trackers do not need frontmatter.
  - User story link: Keeps the documentation layer aligned with the new workflow.
  - Depends on: Tasks 3-7
  - Validate with: `rg -n "shipglowz_data/workflow/BUGS.md|shipglowz_data/workflow/TEST_LOG.md|bug dossier|test-evidence|tracker" skills/sg-docs/SKILL.md`
...
- [ ] Task 9: Update help command coverage
  ...
  - Action: Add the professional bug loop and the roles of `shipglowz_data/workflow/TEST_LOG.md`, `shipglowz_data/workflow/BUGS.md`, `shipglowz_data/workflow/bugs/BUG-ID.md`, and `test-evidence/BUG-ID/`.
  - User story link: A fresh user can discover the workflow without reading implementation skills.
  - Depends on: Tasks 3-8
  - Validate with: `rg -n "bug dossier|shipglowz_data/workflow/TEST_LOG.md|shipglowz_data/workflow/BUGS.md|test-evidence|sg-test --retest" skills/sg-help/SKILL.md`
...
- [ ] Task 10: Update `sg-test` README
  ...
  - Validate with: `rg -n "bug dossier|test-evidence|shipglowz_data/workflow/BUGS.md|shipglowz_data/workflow/TEST_LOG.md|redact" skills/sg-test/README.md`
...
- [ ] Task 12: Update README
  ...
  - Depends on: Task 11
  - Validate with: `rg -n "bug|shipglowz_data/workflow/BUGS.md|shipglowz_data/workflow/TEST_LOG.md|test-evidence|shipglowz_data/workflow/bugs/" README.md`
...
- [ ] Task 13: Update public `sg-test` skill page
  ...
  - Action: Align the public page with compact `shipglowz_data/workflow/TEST_LOG.md`, compact `shipglowz_data/workflow/BUGS.md`, per-bug dossiers and redacted evidence directories.
  - User story link: Prevents public docs from teaching the old bug model.
  - Depends on: Tasks 10-12
  - Validate with: `rg -n "bug dossier|test-evidence|shipglowz_data/workflow/BUGS.md|shipglowz_data/workflow/TEST_LOG.md" site/src/content/skills/sg-test.md`
...
- [ ] Task 15: Update public verification skill page
  ...
  - Depends on: Task 14
  - Validate with: `rg -n "bug|shipglowz_data/workflow/BUGS.md|blocks|verification" site/src/content/skills/sg-verify.md`
...
- [ ] Task 17: Update public docs skill page
  ...
  - Depends on: Task 16
  - Validate with: `rg -n "bug|documentation coherence|shipglowz_data/workflow/BUGS.md|shipglowz_data/workflow/TEST_LOG.md" site/src/content/skills/sg-docs.md`
...
- [ ] CA 1: Given a failed `sg-test` scenario, when the result is logged, then `shipglowz_data/workflow/TEST_LOG.md` contains a short scenario entry with a pointer to `BUG-ID` and `shipglowz_data/workflow/bugs/BUG-ID.md` contains the detailed reproduction, expected behavior, observed behavior, evidence, current status and next step.
- [ ] CA 2: Given a new bug is created, when `shipglowz_data/workflow/BUGS.md` and `shipglowz_data/workflow/bugs/` already contain bugs for the same UTC date, then the new ID uses a suffix greater than every existing suffix and does not overwrite an existing dossier.
...
- [ ] CA 14: Given a fresh agent opens only `shipglowz_data/workflow/BUGS.md` and `shipglowz_data/workflow/bugs/BUG-ID.md`, when asked to continue work, then it can identify current status, reproduction, attempted fixes, evidence, redaction state and next step without reading chat history.
...
- Migration risk: low to medium, because legacy `shipglowz_data/workflow/BUGS.md` files may already contain details. Mitigation: leave old content valid and apply the new structure only to new or retested bugs unless a project explicitly requests migration.
...
- Concurrency risk: low to medium, because multiple agents could assign IDs. Mitigation: re-read `shipglowz_data/workflow/BUGS.md` and `shipglowz_data/workflow/bugs/` just before writing and retry on collision; stop on repeated conflict.
