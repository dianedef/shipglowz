...
- `docs/` holds public or semi-public references plus owner-skill project records such as raw conversation captures and exploration reports. `archive/` may retain indexed inactive history, but archived content is never active doctrine. Root `specs/` and `research/` are migration sources.
