...
| Conversation naming, status, and cleanup | `309-sg-tasks sessions <project-or-cwd>` | Review unmanaged Codex titles; use `sessions prune <project-or-cwd>` for a dry-run preview of old completed sessions before any confirmed deletion. |
...
| `309-sg-tasks` | `sessions <project-or-cwd>`; `sessions prune <project-or-cwd>`; `name-conversation` | Rename unmanaged titles, preserve forks, and preview safe project-scoped pruning; apply requires exact confirmation and excludes the current thread. |
