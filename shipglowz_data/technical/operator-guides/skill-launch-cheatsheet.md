...
| Content management | `007-sg-content [goal, source, file, or mode]` | `plan`, `repurpose`, `draft`, `enrich`, `audit`, `seo`, `editorial`, `apply`, `ship`; it hands market, GTM, copy, and copywriting review to an explicit `009-sg-marketing` mode. Add `score`, `quality gate`, or `grille projet` when you want project-aware scoring through the shared rubric. |
